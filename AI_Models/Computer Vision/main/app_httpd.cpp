...
// استبدل function الـ index_handler في app_httpd.cpp بهذا الكود:
...
<head>
    ...
    <title>ESP32 Driver Monitor</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <h1>🚗 Driver Behavior Monitor</h1>
        
        <div class="alert" id="alert">
            ⚠️ UNSAFE BEHAVIOR DETECTED!
        </div>
        ...
        <div class="main-content">
            ...
            <div class="prediction-panel">
                ...
                <div class="confidence-value" id="confidence">0.0%</div>
            </div>
        </div>
    </div>

    <script>
        const dangerousStates = ['Drowsy', 'Distracted', 'Phone_Use', 'Smoking', 'Drinking'];
        
        async function updatePrediction() {
            try {
                const response = await fetch('http://' + window.location.hostname + ':82');
                const data = await response.json();
                
                document.getElementById('prediction').textContent = data.label;
                document.getElementById('confidence').textContent = data.score.toFixed(1) + '%';
                
                // Show alert for dangerous behavior
                const alertBox = document.getElementById('alert');
                if (dangerousStates.includes(data.label) && data.score > 50) {
                ...
                if (data.label === 'Normal') {
                    predElement.style.color = '#4ade80';
                } else if (dangerousStates.includes(data.label)) {
                    predElement.style.color = '#ef4444';
                } else {
                    predElement.style.color = '#fbbf24';
                }
